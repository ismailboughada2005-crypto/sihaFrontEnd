  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingAdmin) {
        ...
        setAdmins(admins.map(a => a.id === editingAdmin.id ? { ...a, ...updated } : a));
      } else {
        const newAdmin = await api.admins.create(formData);
        setAdmins([{ ...newAdmin, avatar: formData.nom.split(' ').map(n => n[0]).join('').toUpperCase() }, ...admins]);
      }
  ...
  return (
    <div className="p-8 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        >
          {loading ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <UserPlus className="w-4 h-4" />} {loading ? 'Processing...' : 'Add New Administrator'}
        </button>
      </div>
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-6">
                ...
                <div className="pt-4 flex gap-4">
                  ...
                  >
                    {loading ? <div className="w-6 h-6 border-2 border-white border-t-transparent rounded-full animate-spin mx-auto" /> : (editingAdmin ? 'Update Credentials' : 'Grant Access')}
                  </button>
                </div>
              </form>
            </motion.div>
          ...
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 sm:p-0">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative w-full max-w-md bg-white rounded-[2.5rem] p-10 text-center shadow-2xl">
              ...
              <div className="mt-10 flex gap-4">
                ...
                >
                  {loading ? <div className="w-6 h-6 border-2 border-white border-t-transparent rounded-full animate-spin mx-auto" /> : 'Confirm Revoke'}
                </button>
              </div>
            </motion.div>
          </div>
