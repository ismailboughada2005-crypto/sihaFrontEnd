  ...
  >
    ...
    {badge && (
      <span className={`ml-auto text-[10px] font-black px-2 py-0.5 rounded-full ${
        active ? 'bg-card-bg/20 text-white' : 'bg-primary/5 text-primary'
      }`}>
        {badge}
      </span>
    )}
...
const Sidebar = ({ activeTab, onTabChange, patientCount, doctorCount, staffCount, appointmentCount, userRole, onLogout }) => {
  ...
  const dashItems = [
    { id: 'admin', label: t('adminTerminal'), icon: Monitor, roles: ['admin'] },
    { id: 'doctor', label: t('doctorSuite'), icon: Stethoscope, roles: ['doctor'] },
    { id: 'staff', label: t('staffHub'), icon: ClipboardList, roles: ['staff'] },
  ];

  const mainItems = [
    { id: 'patients', label: t('patients'), icon: Users, roles: ['admin', 'staff'] },
    { id: 'doctors', label: t('doctors'), icon: Stethoscope, roles: ['admin'] },
    { id: 'staff-management', label: t('staff'), icon: Users, roles: ['admin'] },
    { id: 'admins', label: t('admins'), icon: ShieldCheck, roles: ['admin'] },
    { id: 'payments', label: t('payments'), icon: CreditCard, roles: ['admin', 'staff'] },
    { id: 'appointments', label: t('appointments'), icon: Calendar, roles: ['admin', 'staff'] },
    { id: 'doctor-appointments', label: t('myAppointments'), icon: Calendar, roles: ['doctor'] },
    { id: 'analytics', label: t('analytics'), icon: LineChart, roles: ['admin'] },
  ];
  ...
  return (
    <aside className="w-72 bg-card-bg border-r border-card-border hidden lg:flex flex-col sticky top-20 h-[calc(100vh-80px)] transition-colors duration-500">
      <div className="p-8 pb-4 flex-1 flex flex-col overflow-y-auto">
        <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-300 mb-6 ml-2">{t('operationalViews')}</h3>
        ...
        <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-300 mb-6 ml-2">{t('centerManagement')}</h3>
